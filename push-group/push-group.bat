...
set "CURRENT_DIR=%cd%"
if not "%FSHARECHAT_PUSH_GROUP_HOME%" == "" goto gotHome
set "FSHARECHAT_PUSH_GROUP_HOME=%CURRENT_DIR%"
if exist "%FSHARECHAT_PUSH_GROUP_HOME%\push-group.bat" goto okHome
cd ..
set "FSHARECHAT_PUSH_GROUP_HOME=%cd%"
cd "%CURRENT_DIR%"
:gotHome
if exist "%FSHARECHAT_PUSH_GROUP_HOME%\push-group.bat" goto okHome
    echo The FSHARECHAT_PUSH_GROUP_HOME environment variable is not defined correctly
...
echo Using FSHARECHAT_PUSH_GROUP_HOME:   "%FSHARECHAT_PUSH_GROUP_HOME%"
...
set FSHARECHAT_PUSH_GROUP_PATH=%FSHARECHAT_PUSH_GROUP_HOME%
set LOG_FILE=%FSHARECHAT_PUSH_GROUP_HOME%\config\logback.xml
set CONFI_DIR = %FSHARECHAT_PUSH_GROUP_HOME%\config\

for /F %%i in ('dir %FSHARECHAT_PUSH_GROUP_HOME%\lib\ /B') do ( set JAR_NAME=%%i)
echo Push-Group JAR_NAME: "%JAR_NAME%"
...
start javaw -server %JAVA_OPTS% %JAVA_OPTS_SCRIPT% -jar %FSHARECHAT_PUSH_GROUP_HOME%\lib\%JAR_NAME% --spring.config.additional-location=file:.\config\ 
rem > %FSHARECHAT_PUSH_GROUP_HOME%\logs\push-group.log 2<&1
